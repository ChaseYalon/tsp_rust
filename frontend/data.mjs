import { bruteForce } from "./js/bruteforce.mjs";
import { christofidesAlgo, computeMST, nearestNeighbor } from "./js/util_algos.mjs";
...
class ReportGenerator {
     ...
     */
    constructor(trigger, output, ldachek, bfcheck, nncheck, cacheck, pointCount, testCount){
    ...
    async generate(){
        ...
        const labels = ["Algorithm Name", "Execution Time (ms)", "Approx % above optimal*"];
        ...
        const algorithms = [
            {
                name: pointCount < 20 ? "Brute Force": "Minimum Spanning Tree",
                enabled: true,
                isOptimal: true,
                runner: async (pointCount, testCount) => {
                    if (pointCount < 20){
                        ...
                        for(let i = 0; i < testCount; i++){
                            ...
                            cumulativeDist += pathDist(bruteForce(points));
                        }
                        this.bfDist = cumulativeDist;
                    ...
                    for(let i = 0; i < testCount; i++){
                        let points = globalPoints[i];
                        let res = computeMST(points);
                        console.log(res);
                        cumulativeDist += edgePathDist(computeMST(points));
                    }
                    console.log("Cumulative dist", cumulativeDist);
                    this.bfDist = cumulativeDist;
                    let totalTime = performance.now() - start;
                    return { cumulativeDist, totalTime};
                }
...
document.addEventListener('DOMContentLoaded', function() {
    let generator = new ReportGenerator(generatebtn, reptcont, ldachek, bfcheck, nncheck, cacheck, pointCount, testCount);
});